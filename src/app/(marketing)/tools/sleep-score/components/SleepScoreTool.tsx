...
function ResultsScreen({ result, history, onRetake }: { result: SleepScoreResult; history: ScoreEntry[]; onRetake: () => void }) {
  ...
  function handleShare() {
    const text = `My Sleep Score: ${result.total}/100 (${result.label}) \u{1F319} \u2014 Test yours at sleepstackapp.com/tools/sleep-score`;
    if (navigator.share) {
...
/* -------------------------------------------------------------------------- */
/*  Quiz Component (exported)                                                 */
/* -------------------------------------------------------------------------- */

export default function SleepScoreTool() {
